#!/usr/bin/env bash
# Synthesize one module of the FIFO for the Basys 3 (Xilinx 7-series) and report resource usage
# Reads the whole rtl/ directory so submodules resolve; the top is the module you name
# Usage: ./synth_stats.sh <top_module>   (e.g. ./synth_stats.sh sync_fifo)
set -e
TOP="$1"
